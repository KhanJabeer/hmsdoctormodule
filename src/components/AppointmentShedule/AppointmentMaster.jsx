...
export default class Availabilitymaster extends Component {
    ...
    fetch(apiurl + "getdocAppointmentSettings", {
      ...
      .then((responseJson) => {
        ...
        if (
          ...
          responseData[0].details.length > 0
        ) {
  ...
  clinicList = () => {
    ...
    for (let i = 0; i < this.state.clinicData.length; i++) {
      ...
      clinicList.push(
        <option key={i + 1} value={this.state.clinicData[i].clinicId}>
          {this.state.clinicData[i].clinicName}
        </option>
      );
    }
    return clinicList;
  ...
  componentDidMount() {
    Axios({
      ...
      .then((response) => {
        //2.getting json response in another promise function called .then function
      ...
      <div className="uploadmaster">
        <Paper>
              ...
              <Grid items xs={6} md={6}>
                ...
                <div className="Availability-clinic-dropdown">
                  ...
                  <select
                    ...
                    onChange={(data) =>
                      this.getDetailsByClinic(data.target.value)
                    }
                  >
          ...
          {this.state.appDetails.length > 0 && (
            <AppointmentDetails
              sucessProp={() => this.getDetailsByClinic(this.state.clinic)}
