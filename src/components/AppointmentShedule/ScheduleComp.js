...
class ScheduleComp extends Component {
  ...
  validate = () => {
    ...
    if (!this.state.slotduration == null && !this.state.isvip == null) {
      durationError = "Time Format is Invalid";
      vipError = "Time Format is Invalid";
    }

    if (durationError,vipError) {
      this.setState({ durationError:this.state.durationError, vipError:this.state.vipError });
  ...
  sendDetails = (data) => {
    ...
    if (isValid) {
      ...
      fetch(
        apiurl +
          (data.id
            ? "editdocAppointmentSettings"
            : "insertdocAppointmentSettings"),
        {
